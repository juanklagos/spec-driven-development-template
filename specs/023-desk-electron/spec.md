...
7. Dado macOS con Gatekeeper activo, cuando se abre el `.dmg` descargado, entonces la app arranca sin el diálogo de "desarrollador no identificado", porque está firmada y notarizada.
...
- R8. **Empaquetado y firma**: `electron-builder` para macOS (`.dmg`, universal arm64+x64, firmado y notarizado) y Windows (`.exe` NSIS, firmado). Linux `AppImage` y `.deb` sin firma de tienda — Flathub sigue prohibiendo código asistido por IA desde 2026-05-29.
