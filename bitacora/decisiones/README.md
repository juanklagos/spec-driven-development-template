...
### 2026-08-25 — [La pertenencia a un grupo se deriva de la geometría y no se guarda](2026-08-25-la-pertenencia-a-un-grupo-no-se-guarda.md)

- **ES:** JSON Canvas no tiene campo de padre: estar dentro de un grupo es geometría. React Flow exige `parentId` y coordenadas relativas, así que la pertenencia se deriva al cargar y se disuelve al guardar — el archivo solo lleva coordenadas absolutas. Se descarta guardarla, y también `extent: "parent"`, que atraparía las tarjetas dentro del marco (spec 041).
- **EN:** JSON Canvas has no parent field — being inside a group is geometry. React Flow needs `parentId` and relative coordinates, so membership is derived on load and dissolved on save; the file carries absolute coordinates only. Storing it is rejected, and so is `extent: "parent"`, which would trap cards inside the frame (spec 041).
