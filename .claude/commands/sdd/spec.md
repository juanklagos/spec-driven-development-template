---
description: Create or refine a numbered spec bundle (spec, plan, tasks, history) / Crea o refina un paquete de spec numerado (spec, plan, tasks, history)
argument-hint: [feature name or spec number]
---

Create or refine a spec bundle. Respond in the user's language (EN/ES).

1. If `$ARGUMENTS` names a new feature: run `./scripts/new-spec.sh "<feature>" "<owner>"` (sidecar: `./spec/scripts/new-spec.sh`). If it names an existing spec (e.g., `002`), open that bundle.
...
4. If the spec is ready, ask the user to approve it and record the approval (status, date, approver, evidence) in `spec.md`.
5. Update `specs/INDEX.md` (status/priority/date) and point to `/sdd:gate` as the next step.
