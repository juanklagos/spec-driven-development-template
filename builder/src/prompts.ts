...
// (Claude Code, Codex, Cursor, ...). Both prompts are bilingual ES/EN.
...
export function buildOrchestratorPrompt(description: string, projectRoot: string): string {
  const idea = description.trim() || "[describe tu proyecto / describe your project]";
  const root = projectRoot || "[ruta del workspace / workspace path]";
  return [
...
export function buildImplementPrompt({ projectRoot, specId, specDir }: ImplementPromptInput): string {
  return [
