...
### Added
- **Pedirle ayuda a la IA ya no pasa por el portapapeles.** El builder generaba prompts que había que copiar a la terminal y pegar la respuesta de vuelta: la dirección agente → lienzo estaba resuelta desde la spec 028, pero la de vuelta seguía siendo trabajo manual. Ahora todo campo editable de contenido — las 7 secciones de `spec.md`, el texto de las tareas, las notas del lienzo y los borradores de bitácora — tiene un botón **Ampliar con IA**: el builder publica una petición en `.sdd/requests/`, la sesión de agente del usuario la reclama por MCP y responde con una propuesta que llega sola al lienzo como diff, con Aceptar y Rechazar. Dos tools nuevas (35 → 37): `sdd_next_request` y `sdd_respond_request`. El asistente ✨ acepta además una idea en bruto y la devuelve como borrador de spec estructurado (historia, escenarios, criterios EARS, requisitos), editable antes de crear nada. Sin agente escuchando, cada botón lo dice en el momento y ofrece el prompt copiable de siempre — nadie se queda esperando. (spec 031)
- **La IA propone; solo el humano escribe.** El módulo de la cola no importa ningún escritor de specs: la garantía es estructural, no disciplinaria. Una propuesta nunca toca `specs/`; la única escritura ocurre cuando la persona acepta, y va por las rutas ya probadas (secciones, tareas, canvas, bitácora). Los campos de aprobación y consentimiento quedan explícitamente sin botón de IA — son la firma humana de la compuerta — y una prueba lo verifica por ausencia. (spec 031)
- **Conectar el agente pasó de siete recetas a un comando.** Registrar el MCP significaba encontrar el archivo correcto de cada herramienta, con cuatro claves raíz distintas y dos formatos, y luego escribir a mano el prompt del bucle en cada sesión. `sdd-mcp connect` detecta los clientes presentes y escribe la configuración en el archivo propio de cada uno — Claude Code, Codex, Cursor, VS Code, Windsurf, Gemini CLI y opencode — e instala la skill `/sdd-serve` que atiende la cola. **Fusiona, nunca plantilla**: conserva el resto de la configuración, y un archivo que no se puede interpretar se deja intacto y se reporta en vez de sobrescribirse. Ejecutarlo dos veces no cambia nada. Trae `--dry-run`, `--client`, `--global` y `--project-root`. (spec 032)
- **Tres caminos para atender la cola, porque en 2026 ninguno cubre a todos.** La skill portable `/sdd-serve` (estándar abierto SKILL.md: Claude Code, Codex, Cursor y compatibles), comandos nativos para los que no lo leen (TOML de Gemini CLI, markdown de opencode) y el prompt MCP `sdd_serve_requests`, que no necesita instalar nada en los clientes que muestran prompts MCP como slash commands — Codex todavía no. El texto del bucle tiene una sola fuente en `sdd-core`, así que los tres no pueden divergir. Dentro del builder, **⌘K → Conectar agente** muestra el comando con la ruta ya puesta y, por cliente, la configuración exacta. (spec 032)

### Fixed
- **Una tabla TOML al final del archivo se duplicaba.** El merge de configuración de Codex usaba `\Z` para "fin de documento", que en regex de JavaScript no existe: se interpretaba como una «Z» literal, así que `[mcp_servers.sdd]` no se detectaba si era la última tabla y se añadía otra vez. Con eso, un salto de línea extra rompía además la idempotencia: cada pasada reportaba "actualizado" sobre un archivo que no había cambiado.
- **El pin exacto entre paquetes convertía cada release en una ruleta.** `@juanklagos/sdd-mcp` declaraba `"@juanklagos/sdd-core": "2.3.0"` con versión exacta. Al subir el núcleo a 2.4.0, npm dejó de considerar el tarball local y bajó el 2.3.0 del registro, así que el paquete instalado moría al arrancar con `does not provide an export named …` — el mismo síntoma que tendría cualquiera que hiciera `npx` el día de la publicación. Lo cazó `scripts/smoke-test-npm-package.mjs`, que instala tarballs de verdad en un proyecto limpio; el pin se sube ahora junto con la versión.
- **En proyectos con sidecar, la configuración iba al sitio equivocado.** `connect` usaba la raíz SDD resuelta como destino, que en un sidecar es `<proyecto>/spec`: escribía ahí las configuraciones de cliente y registraba esa ruta como `SDD_PROJECT_ROOT`. Ahora la raíz SDD solo valida que el workspace exista; lo que se registra y donde se escribe es la raíz del proyecto, que es donde los clientes buscan.
...
### Added
- **`npx @juanklagos/sdd-mcp --http`** starts the board from the bin that carries the package name, so getting there no longer depends on npx resolving a second executable. The `sdd-mcp-http` bin stays — it is documented outside this repo and sits in people's MCP configs.

### Changed
- **`QUICKSTART.md` opens with the one-command scaffolder.** The door advertised as "about five minutes" ran to eight steps and never mentioned `create-sdd-project`, which is published and works. Cloning the whole template is now the explicitly longer route, for people who want the guides in the same repository as their code.
