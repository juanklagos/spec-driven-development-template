...
export async function createSpec(input) {
    ...
    for (const file of ["spec.md", "plan.md", "tasks.md", "research.md", "history.md"]) {
...
export async function validateProject(projectRoot) {
    ...
    for (const dir of ["bitacora/global", "bitacora/diaria", "bitacora/handoffs", "bitacora/decisiones"]) {
...
export async function checkGate(projectRoot) {
    const root = path.resolve(projectRoot);
    await ensureProjectRootAllowed(root);
    const messages = [];
    const specs = await listSpecs(root);
...
export async function recordUserConsent(projectRoot, summary) {
    const root = path.resolve(projectRoot);
    await ensureProjectRootAllowed(root);
...
export async function generateStatus(projectRoot) {
    const root = path.resolve(projectRoot);
    await ensureProjectRootAllowed(root);
...
export async function generateRoadmap(projectRoot) {
    const root = path.resolve(projectRoot);
    await ensureProjectRootAllowed(root);
    const indexPath = path.join(root, "specs/INDEX.md");
...
export async function appendProjectLogEntry(projectRoot, entry) {
    const root = path.resolve(projectRoot);
    await ensureProjectRootAllowed(root);
...
export async function writeDailyLog(projectRoot, date, content) {
    const root = path.resolve(projectRoot);
    await ensureProjectRootAllowed(root);
...
export async function writeHandoff(projectRoot, fileName, content) {
    const root = path.resolve(projectRoot);
    await ensureProjectRootAllowed(root);
...
export async function writeDecision(projectRoot, fileName, content) {
    const root = path.resolve(projectRoot);
    await ensureProjectRootAllowed(root);
...
export async function ensureProjectRootAllowed(projectRoot) {
    const root = path.resolve(projectRoot);
...
async function requireDir(root, relativePath, messages) {
...
async function requireFile(root, relativePath, messages) {
    if (!(await exists(path.join(root, relativePath)))) {
        messages.push({
            level: "error",
            code: "missing-file",
